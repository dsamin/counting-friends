...
// Native (Capacitor) builds set CAP_BUILD=1. We disable vite-plugin-pwa for them:
// a service worker inside the WKWebView causes stale-cache white-screens and is
// redundant (the native app already bundles every asset locally). The web/PWA build
// leaves CAP_BUILD unset, so the service worker + manifest stay on as before.
